...
" + "\n" +
                    ...
                    if (item.TELEFON.StartsWith("+9"))
                    {
                        ...
                        if (errorApiDial.Select(x => x.CustPhone).Contains(item.TELEFON.Substring(2)))
                        {
                            if (errorApiDial.Find(x => x.CustPhone == item.TELEFON.Substring(2)).DurumType != true)
                            {
                                ...
                                {
                                    ApiAddToCampaign("", "", item.AD_SOYAD, item.TELEFON.Substring(2), null, "POLLOB-" + cb_pollob.Text, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), endTime.ToString("yyyy-MM-dd HH:mm:ss"));
                                }
                                else if ((DateTime.Now > startTime2 && DateTime.Now < endTime2))
                                {
                                    ApiAddToCampaign("", "", item.AD_SOYAD, item.TELEFON.Substring(2), null, "POLLOB-" + cb_pollob.Text, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), endTime2.ToString("yyyy-MM-dd HH:mm:ss"));
                                }
                    ...
                    else if (!item.TELEFON.StartsWith("0"))
                    {
                        if (errorApiDial.Select(x => x.CustPhone).Contains(item.TELEFON.Substring(2)))
                        {
                            if (errorApiDial.Find(x => x.CustPhone == item.TELEFON.Substring(2)).DurumType != true)
                            {
                                if ((DateTime.Now > startTime && DateTime.Now < endTime))
                                {
                                    ApiAddToCampaign("", "", item.AD_SOYAD, "0"+item.TELEFON, null, "POLLOB-" + cb_pollob.Text, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), endTime.ToString("yyyy-MM-dd HH:mm:ss"));
                                }
                                else if ((DateTime.Now > startTime2 && DateTime.Now < endTime2))
                                {
                                    ApiAddToCampaign("", "", item.AD_SOYAD, "0"+item.TELEFON, null, "POLLOB-" + cb_pollob.Text, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), endTime2.ToString("yyyy-MM-dd HH:mm:ss"));
                                }
                    ...
                    else
                    {
                        ...
                        if (errorApiDial.Select(x => x.CustPhone).Contains(item.TELEFON))
                        {
                            if (errorApiDial.Find(x => x.CustPhone == item.TELEFON).DurumType != true)
                            {
                                if ((DateTime.Now > startTime && DateTime.Now < endTime))
                                {
                                    ApiAddToCampaign("", "", item.AD_SOYAD, item.TELEFON, null, "POLLOB-" + cb_pollob.Text, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), endTime.ToString("yyyy-MM-dd HH:mm:ss"));

                                }
                                else if ((DateTime.Now > startTime2 && DateTime.Now < endTime2))
                                {
                                    ApiAddToCampaign("", "", item.AD_SOYAD, item.TELEFON, null, "POLLOB-" + cb_pollob.Text, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), endTime2.ToString("yyyy-MM-dd HH:mm:ss"));
                                }
                            }
        ...
        private void startWithQuartz(object sender, ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    notificatonSettings("Uygulama çalışma saatini geçti \n Uygulama işlem saatleri 08:30 / 12:00 && 13:00 / 17:00");
